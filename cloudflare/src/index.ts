...
// Session cache: maps session ID → transport for the lifetime of the isolate.
const sessions = new Map<string, WorkerTransport>();
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // New session: create a fresh server + transport so the MCP protocol
    // state machine starts clean (avoids "already initialized").
