#!/usr/bin/env python3
"""
UAB Research Computing Documentation MCP Server (Smithery Version)
This version uses the Smithery SDK for streamable HTTP compatibility.
"""
...
class ConfigSchema(BaseModel):
    githubToken: str = Field(
        default="",
        description="Optional GitHub Personal Access Token for higher API rate limits (5000/hour vs 60/hour)"
    )

# Initialize FastMCP server
mcp_base = FastMCP("uab-research-computing-docs")

# Wrap with Smithery for streamable HTTP support
mcp = from_fastmcp(mcp_base, config_schema=ConfigSchema)


def get_github_token() -> str:
    """Get GitHub token from session config or environment."""
    try:
        ctx = mcp.get_context()
        ...
        logger.debug(f"Could not get token from session config: {e}")
...
if __name__ == "__main__":
    ...
    logger.info("Starting UAB Research Computing Documentation MCP Server (Smithery)")
    
    # Get port from environment (Smithery sets PORT=8081)
    port = int(os.environ.get("PORT", "8081"))
    
    logger.info(f"Server will listen on port {port}")
    logger.info("Using Smithery SDK for streamable HTTP support")
